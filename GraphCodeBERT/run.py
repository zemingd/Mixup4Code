        ...
        try:
            ...
            with open(file_path) as f:
                for line in f:
                    ...
                    self.examples.append(convert_examples_to_features(code, label, tokenizer, args))
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results['eval_precision'] > best_f1:
                        ...
                        output_dir1 = f"{root_path}/{args.output_dir}"
                        if not os.path.exists(output_dir1):
                            os.makedirs(output_dir1)
                        model_to_save = model.module if hasattr(model, 'module') else model
                        output_dir = os.path.join(output_dir1, '{}'.format('model.bin'))
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    eval_acc = np.mean(y_trues == np.array(y_preds))
    result = {
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--data_dir", default="datasets/GraphCodeBERT/code_classification", type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    parser.add_argument("--output_dir", default="models/GraphCodeBERT/code_classification/model", type=str,
                        help="The output directory where the model predictions and checkpoints will be written.")
    ...
    args.device = device
    args.train_data_file = f"{root_path}/{args.data_dir}/train.jsonl"
    args.eval_data_file = f"{root_path}/{args.data_dir}/test.jsonl"
    args.test_data_file = f"{root_path}/{args.data_dir}/test.jsonl"
    ...
    # Evaluation
    if args.do_eval:
        checkpoint_prefix = f"{root_path}/{args.output_dir}/model.bin"

...
def main():
    ...
    class Net(torch.nn.Module):
        def __init__(self, out_channels, num_layers, Vertex_feature_dim):
            super().__init__()
            self.out_channels = out_channels
            self.num_layers = num_layers
            self.Vertex_feature_dim = Vertex_feature_dim
            self.conv1 = GatedGraphConv(out_channels, num_layers)
    ...
    model = Net(out_channels, num_layers, Vertex_feature_dim).to(device)
